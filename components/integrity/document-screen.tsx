...
import { extractDocumentData } from "@/lib/process-document"
...
export function DocumentScreen({ onNavigate, onDocumentExtracted }: DocumentScreenProps) {
  ...
  const [isProcessing, setIsProcessing] = useState(false) // Replaces uploadProgress
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      if (data) {
        // Map Gemini JSON to your ProjectRecord type
        const record: ProjectRecord = {
          ...
          location: {
            lat: 19.076, // Mocking lat/long for now as documents rarely have coords
            lng: 72.8777,
      ...
      alert("Failed to read document. Please try a clear image or PDF.")
    } finally {
  ...
  // Helper to map data for display
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-120px)]">
              ...
              >
                ...
                {isProcessing && (
                  <div className="w-full max-w-md px-8 animate-pulse">
                    <p className="font-mono text-xs tracking-[0.2em] uppercase text-accent mb-4 text-center">
                      ANALYZING WITH GEMINI AI...
                    </p>
                    <div className="h-1 bg-border w-full overflow-hidden">
                      <div className="h-full bg-accent w-1/2 animate-[shimmer_1s_infinite]" />
                    </div>
                  </div>
                )}
          ...
          <div className="w-full lg:w-1/2 p-6 md:p-12 bg-background">
            ...
            <div className="border border-border">
             ...
             {/* Action buttons (Keep your existing buttons here) */}
             {extractedData && (
              <div className="mt-8 flex flex-col gap-3">
                <button
                  onClick={() => onNavigate("evidence")}
                  className="w-full px-6 py-4 border border-foreground font-mono text-xs tracking-[0.15em] uppercase hover:bg-foreground hover:text-background transition-all duration-200"
                >
